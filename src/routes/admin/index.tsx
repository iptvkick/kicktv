...
function AdminDashboardPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const receita = payments?.reduce((acc: number, curr: any) => acc + Number(curr.valor), 0) || 0;
  ...
  const { ativosCount, trialsCount, receita, ultimosRegistros, servers } = data;
  const totalUsuarios = ativosCount + trialsCount;
  const taxaConversao = totalUsuarios > 0 ? Math.round((ativosCount / totalUsuarios) * 100) : 0;

  const stats = [
    { label: "Usuários Ativos", value: ativosCount.toString(), icon: Users },
    ...
    { label: "Trials Ativos", value: trialsCount.toString(), icon: Activity },

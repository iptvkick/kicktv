...
export default function ClienteDashboardPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Fetching apenas o necessário para evitar erros com queries aninhadas inexistentes
  ...
  // Skeleton UI (Impede a tela branca de "mil anos")
  if (loading) {
    return (
      <div className="flex flex-col pt-12 px-6 w-full gap-8 animate-pulse">
  ...
  return (
    <div className="flex flex-col pt-12 px-6">
      ...
      {/* Hero Card SDD Clean */}
      <div className="relative w-full aspect-[4/5] bg-gradient-to-tr from-[#212529] to-[#343a40] rounded-[24px] overflow-hidden shadow-xl p-6 flex flex-col justify-end text-white">
        ...
        <div className="relative z-10 flex flex-col gap-2">
          <div className="flex items-center gap-2 mb-2">
            <span className={`h-2 w-2 rounded-full ${isActive ? 'bg-green-400 animate-pulse shadow-[0_0_10px_#4ade80]' : 'bg-red-400'}`} />
          ...
          <h2 className="text-4xl font-bold mb-1 tracking-tight">
            {subscription?.plans?.name || 'KickTV Premium'}
          </h2>
          ...
          {isActive ? (
            <div className="mt-6 flex flex-col gap-3">
              <div className="bg-white/10 backdrop-blur-xl rounded-xl p-3 flex justify-between items-center border border-white/20">
                <div className="flex flex-col">
                  <span className="text-[10px] uppercase text-white/50 font-bold tracking-wider">Usuário M3U</span>
                  <span className="font-mono text-sm">{profile?.email?.split('@')[0]}</span>
                </div>
                <button className="text-xs bg-white/20 hover:bg-white/30 px-4 py-2 rounded-lg font-medium transition-colors">Copiar</button>
              </div>
              <div className="bg-white/10 backdrop-blur-xl rounded-xl p-3 flex justify-between items-center border border-white/20">
                <div className="flex flex-col">
                  <span className="text-[10px] uppercase text-white/50 font-bold tracking-wider">Senha M3U</span>
            ...
            <div className="mt-6 p-4 bg-red-500/20 border border-red-500/30 rounded-xl">
              <p className="text-sm text-red-100">Sua assinatura expirou. Renove agora para continuar assistindo.</p>
            </div>
          )}
        </div>
      </div>
